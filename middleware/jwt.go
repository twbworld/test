...
func JWTAuth(ctx *gin.Context) {
	...
	if err != nil || now.After(et.Time) {
		...
		return
	} else if audience, err := claims.GetAudience(); err != nil || utils.InSlice(audience, `miniProgram`) < 0 {
	...
	if nb, err := claims.GetNotBefore(); (err != nil && nu+60*10 > eu) || (err == nil && 2*nu > eu+nb.Unix()) {
		...
		if dao.App.UserDb.GetUserById(u, uint(userId)) != sql.ErrNoRows && u.Id > 0 {
			t, err = service.Service.UserServiceGroup.BaseService.LoginToken(u)
			if err == nil && len(t) > 0 {
				rw = &reWriter{
					ctx.Writer,
					&bytes.Buffer{},
