...
func (d *DatingDb) GetUserTimeById(userTime *db.UserTime, UtId uint, tx ...*sqlx.Tx) error {
	sql := fmt.Sprintf("SELECT `id`, `dating_id`, `user_id` FROM `%s` WHERE `id` = ?", userTime.TableName())
	if len(tx) > 0 && tx[0] != nil {
...
func (d *DatingDb) GetDatingUsers(datingUsers *[]common.DatingUser, datingId uint, tx ...*sqlx.Tx) error {
	sql := fmt.Sprintf("SELECT `u`.`id`, `u`.`nick_name`, `u`.`avatar_url`, `ut`.`id` AS ut_id, `ut`.`info` FROM `%s` AS ut JOIN `%s` AS u ON `ut`.`dating_id`= ? AND `ut`.`user_id` = `u`.`id` AND `ut`.`status` = 1 ORDER BY `ut`.`id`", db.UserTime{}.TableName(), db.User{}.TableName())
	if len(tx) > 0 && tx[0] != nil {
...
func (d *DatingDb) GetUserByDatingIds(datingUserAvatar *[]common.DatingUserAvatar, datingIds []uint, tx ...*sqlx.Tx) error {
	...
	sql := fmt.Sprintf("SELECT `ut`.`dating_id`, `u`.`avatar_url` FROM `%s` AS ut JOIN `%s` AS u ON `ut`.`dating_id` IN (?) AND `ut`.`user_id` = `u`.`id` AND `ut`.`status` = 1", db.UserTime{}.TableName(), db.User{}.TableName())
...
func (d *DatingDb) JoinDating(UserTime *db.UserTime, tx *sqlx.Tx) (uint, error) {
	...
	if err != nil {
		return 0, errors.New("[fuisdu]" + err.Error())
	}
	id, err := res.LastInsertId()
	return uint(id), err
}
...
func (d *DatingDb) JoinUpdate(utId uint, UserTime *db.UserTime, tx *sqlx.Tx) (err error) {
	...
	if CanLock {
		...
		if _, err = tx.Exec(sql, utId); err != nil {
			return errors.New("[fuisdu]" + err.Error())
		}
	}
