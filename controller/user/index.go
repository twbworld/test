...
func (d *DatingApi) GetDating(ctx *gin.Context) {
	...
	for key, value := range datingUsers {
		...
		if info.Time == nil || len(info.Time) < 1 {
			...
			datingUsers[key].InfoResponse.Time = make([][]string, 0)
		}
		for _, val := range info.Time {
			...
			datingUsers[key].InfoResponse.Time = append(datingUsers[key].InfoResponse.Time, []string{utils.TimeFormat(val[0]), utils.TimeFormat(val[1])})
...
func (d *DatingApi) GetDatingAmount(ctx *gin.Context) {
	...
	if err = dao.App.DatingDb.GetDatingAmount(&datingIds, userId); err != nil && err != sql.ErrNoRows {
...
func (d *DatingApi) GetDatingList(ctx *gin.Context) {
	...
	for _, value := range datingUserAvatar {
		...
		datingKeyUserAvatar[value.DatingId] = append(datingKeyUserAvatar[value.DatingId], value.AvatarUrl)
	}
...
func (d *DatingApi) QuitDating(ctx *gin.Context) {
	...
	if dao.App.DatingDb.GetDating(&dating, data.Id) != nil || dating.Status == 0 {
	...
	err = d.tx(dating.Id, func(tx *sqlx.Tx) (e error) {
		if !isUtId && dating.CreateUserId == userId {
			//关闭(创建者主动 退出或关闭 会面)
			e = dao.App.DatingDb.CloseDating([]uint{dating.Id}, tx)
		} else {
			//退出
			e = dao.App.DatingDb.QuitDating(userTime.Id, tx)
		}
		return
	})
	if err != nil {
